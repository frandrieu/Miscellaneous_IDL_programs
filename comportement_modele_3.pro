...
;               
;force_mima : force min et max à 0 et 1 sur les graphes
;               force_mima=0 : désactivé
;               force_mima=1 : activé
;              
par=2
plot_option=1
plot_name='/home/fandrieu/Inversion/Tests_model/Comportement_model_c_4.png'
force_mima=0
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
types=['n=1.5 ; k=1e-3', 'n=1.5 ; k=1e-9', 'n=1.1 ; k=1e-3']
uncon='n=1.3 ; k=1e-6'
ntypes=n_elements(types)+0.01
;ntypes=4.01
;;;;;; X ;;;;;;;;;;;;;;
...
'   p='+string((1.-c)*1.e6, format='(f7.0)')+'ppm'+'   g='+string(g, format='(f5.0)')+string(181B)+'m', yrange=[0.,mma], ystyle=1 
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.)
endif
if par eq 2 then begin
...
'   h='+string(h, format='(f4.0)')+'mm'+'   g='+string(g, format='(f5.0)')+string(181B)+'m', yrange=[mmi,mma];,yrange=[min(comport), max(comport)]
oplot, 1.e6*x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.)
endif
if par eq 3 then begin
...
'   e= '+string(emer, format='(f4.1)')+string(176B)+'   !9y'+'!1 = '+string(azim, format='(f4.0)')+string(176B)+$
'   h='+string(h, format='(f4.0)')+'mm'+'   p='+string((1.-c)*1.e6, format='(f7.0)')+'ppm', yrange=[mmi,mma];, yrange=[min(comport)-0.05, max(comport)]
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.)
endif
if par eq 2 then begin
legend, ['Inclusions type :'], thick=3, pos=[280, 820],CHARTHICK=2,CHARSIZE=2,COLORS=0,TEXTCOLORS=0,OUTLINE_COLOR=255,/device
legend, types[0], psym=0, thick=3, pos=[280, 780],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/ntypes*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif else begin
legend, ['Inclusions type :'], thick=3, pos=[780, 820],CHARTHICK=2,CHARSIZE=2,COLORS=0,TEXTCOLORS=0,OUTLINE_COLOR=255,/device
legend, types[0], psym=0, thick=3, pos=[780, 780],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/ntypes*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endelse
...
if par eq 1 then begin
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.)
legend, types[i], psym=0, thick=3, pos=[780, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/ntypes*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif
if par eq 2 then begin
oplot, 1.e6*x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.)
legend, types[i], psym=0, thick=3, pos=[280, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/ntypes*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif
if par eq 3 then begin
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/ntypes*255.) 
legend, types[i], psym=0, thick=3, pos=[780, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/ntypes*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
...
endelse
  ...
  endrep until (coun ge n_elements(types)+1)
;;;;;; Z ;;;;;;;;;;;;;;
if plot_option eq 2 then begin
plot_file=plot_name
t=tvrd(true=1)
