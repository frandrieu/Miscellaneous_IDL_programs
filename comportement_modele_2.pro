...
par=2
plot_option=1
plot_name='/home/fandrieu/Inversion/Tests_model/Comportement_model_h_4.png'
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
types=['n=1 ; k=1e-16', 'n=1.5 ; k=1e-3', 'n=1.9 ; k=1e-9', 'n=1.1 ; k=1e-3','n=1.25 ; k=1e-9']
uncon='n=1.3 ; k=1e-6'
...
coun=0
repeat begin
  for i=0,n_elements(types) do begin
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ...
    if i eq 2 then begin
      ;rough=0.5 ouverture source 0.02° ouverture observateur 4.2° (spectro IPAG)
      path_spectres = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_h_1_neq_keq/comportement_model_0.5_h_1_neq_keq_em0_C.cub'
      envi_open_file, path_spectres, r_fid=data_fid_h
      envi_file_query, data_fid_h, dims=dims_data_h, wl=wl, nb=nb
      cub_h=fltarr((dims_data_h[2]+1)*(dims_data_h[4]+1),nb)
      for ii=0,nb-1 do cub_h[*,ii] = ENVI_GET_DATA(fid=data_fid_h, dims=dims_data_h, pos=[ii] )
      ...
      npix_h=(dims_data_h[2]+1)*(dims_data_h[4]+1)
      path = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_h_1_neq_keq/comportement_model_0.5_h_1_neq_keq_em0.param'
      ...
      ;rough=0.5 ouverture source 0.02° ouverture observateur 4.2° (spectro IPAG)
      path_spectres = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_c_1_neq_keq/comportement_model_0.5_c_1_neq_keq_em0_C.cub'
      envi_open_file, path_spectres, r_fid=data_fid_c
      envi_file_query, data_fid_c, dims=dims_data_c, wl=wl, nb=nb
      cub_c=fltarr((dims_data_c[2]+1)*(dims_data_c[4]+1),nb)
      for ii=0,nb-1 do cub_c[*,ii] = ENVI_GET_DATA(fid=data_fid_c, dims=dims_data_c, pos=[ii] )
      ...
      npix_c=(dims_data_c[2]+1)*(dims_data_c[4]+1)
      path = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_c_1_neq_keq/comportement_model_0.5_c_1_neq_keq_em0.param'
      ...
      ;rough=0.5 ouverture source 0.02° ouverture observateur 4.2° (spectro IPAG)
      path_spectres = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_g_1_neq_keq/comportement_model_0.5_g_1_neq_keq_em0_C.cub'
      envi_open_file, path_spectres, r_fid=data_fid_g
      envi_file_query, data_fid_g, dims=dims_data_g, wl=wl, nb=nb
      cub_g=fltarr((dims_data_g[2]+1)*(dims_data_g[4]+1),nb)
      for ii=0,nb-1 do cub_g[*,ii] = ENVI_GET_DATA(fid=data_fid_g, dims=dims_data_g, pos=[ii] )
      ...
      npix_g=(dims_data_g[2]+1)*(dims_data_g[4]+1)
      path = '/home/fandrieu/Inversion/Simulations/comportement_model_0.5_g_1_neq_keq/comportement_model_0.5_g_1_neq_keq_em0.param'
    ...
    if i eq n_elements(types) then begin
    
    if par eq 1 then begin
      w_simu=where(inc_simu eq incid and emer_simu eq emer and azim_simu eq azim and $
        c_simu eq 1. and g_simu eq g)
      cub_simu_geom=cub_simu[w_simu,*]
      comport=cub_simu_geom[*,num_wave]
      x_axis=h_x
    endif
...
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/5.01*255.)
endif
if par eq 2 then begin
...
if par eq 3 then begin
plot, x_axis,comport, charsize=2, charthick=2, xtitle='Radius of the inclusions ('+string(181B)+'m)', thick=3, $
...
legend, types[0], psym=0, thick=3, pos=[280, 520],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/5.01*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif else begin
legend, ['Inclusions type :'], thick=3, pos=[780, 820],CHARTHICK=2,CHARSIZE=2,COLORS=0,TEXTCOLORS=0,OUTLINE_COLOR=255,/device
legend, types[0], psym=0, thick=3, pos=[780, 780],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/5.01*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endelse
endif else begin
if i lt n_elements(types) then begin
if par eq 1 then begin
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/5.01*255.)
legend, types[i], psym=0, thick=3, pos=[780, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/5.01*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif
if par eq 2 then begin
oplot, 1.e6*x_axis,comport, thick=3, color=fix(float(i+1)/5.01*255.)
legend, types[i], psym=0, thick=3, pos=[280, 520-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/5.01*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif
if par eq 3 then begin
oplot, x_axis,comport, thick=3, color=fix(float(i+1)/5.01*255.) 
legend, types[i], psym=0, thick=3, pos=[780, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=fix(float(i+1)/5.01*255.),TEXTCOLORS=0,OUTLINE_COLOR=255,/device
...
if par eq 1 then begin
oplot, x_axis,comport, thick=3, color=0
legend, ['Uncontaminated matrix :'], thick=3, pos=[780, 780-i*40],CHARTHICK=2,CHARSIZE=2,COLORS=0,TEXTCOLORS=0,OUTLINE_COLOR=255,/device
legend, uncon, psym=0, thick=3, pos=[780, 780-i*40-40],CHARTHICK=2,CHARSIZE=2,COLORS=0,TEXTCOLORS=0,OUTLINE_COLOR=255,/device
endif
endelse
endelse
endfor

  endrep until (coun ge 6)
;;;;;; Z ;;;;;;;;;;;;;;
if plot_option eq 2 then begin
plot_file=plot_name
t=tvrd(true=1)
