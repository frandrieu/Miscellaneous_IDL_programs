...
pro invers_spectres_valid_num_sig
...
l_h_fin=dblarr(n_tir,n_elements(h_x),n_elements(sigma_n))
l_g_fin=dblarr(n_tir,n_elements(g_x),n_elements(sigma_n))
t=systime(1)
for noise_loop=0,n_elements(sigma_n)-1 do begin
sigma=sigma_n[noise_loop]
for num_inv=0, n_elements(h)-1 do begin

ggd=  where(abs(param_cub_simu[0,*,1]-g[num_inv]) lt 10. and abs(param_cub_simu[0,*,2]-h[num_inv]) lt 0.001)
pos[num_inv]=ggd[0]
data=cub_simu[0,pos[num_inv],*]
...
for ii=0,n_elements(g_x)-1 do begin
  l_g[ii]=0.
  w=where(abs(param_cub_simu[0,*,1]-g_x[ii]) lt 1.)
...
l_h_fin[*,*,noise_loop]=l_h_tot
l_g_fin[*,*,noise_loop]=l_g_tot
print, 'noise=', sigma
endfor

time=systime(1)-t
print, "inversion time : ", time 
name=strcompress('/home/fandrieu/Inversion/valid_num_results_sigma_h'+$
string(h, format="(F4.1)")+'_g'+string(g, format="(F5.1)")+'_10tirages.sav', /remove_all)
save, time, param_cub_simu, h_x, g_x, l_h_fin, l_g_fin, filename=name
print, name
